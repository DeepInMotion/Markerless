...
def SEBlock(block_args, global_params, inference=False):
    ...
    def block(inputs):
        ...
        if not inference:
            ...
            x = KL.Lambda(lambda x: K.sigmoid(x) * x)(x)
        # Excite
        x = KL.Conv2D(
            filters,
            kernel_size=[1, 1],
...
def MBConvBlock(block_args, global_params, drop_connect_rate=None, inference=False):
    ...
    def block(inputs):
        ...
        if has_se:
            x = SEBlock(block_args, global_params, inference=inference)(x)
        ...
        if block_args.id_skip:
            if all(
                ...
                if drop_connect_rate and not inference:
                    x = DropConnect(drop_connect_rate)(x)
                else:
                    x = KL.Lambda(lambda x: x)(x)
                x = KL.Add()([x, inputs])
        return x
...
def EfficientNet(input_shape, block_args_list, global_params, input_tensor=None, include_top=True, pooling=None, initial_inputs=None, num_blocks=None, inference=False):
    batch_norm_momentum = global_params.batch_norm_momentum
    batch_norm_epsilon = global_params.batch_norm_epsilon
    if global_params.data_format == 'channels_first':
        channel_axis = 1
    ...
    blocks = len(block_args_list) if num_blocks is None else num_blocks
    for i in range(blocks):
        block_args = block_args_list[i]
        ...
        x = MBConvBlock(block_args, global_params,
                        drop_connect_rate=drop_rate_dx * block_idx, inference=inference)(x)
        block_idx += 1
    ...
    if initial_inputs is None:
        ...
        model = KM.Model(initial_inputs, outputs)

    return model


def _get_model_by_name(model_name, input_shape=None, input_tensor=None, include_top=True, weights=None, classes=1000, pooling=None, initial_inputs=None, num_blocks=None, inference=False):
    ...
    Args:
        ...
        initial_inputs: optional, if ``None`` default_input_tensor is used
        num_blocks: optional, if ``None`` default EfficientNet is used
        inference: optional, if ``True`` DropConnect layer is ignored
    ...
    model = EfficientNet(input_shape, block_agrs_list, global_params, input_tensor=input_tensor, include_top=include_top, pooling=pooling, initial_inputs=initial_inputs, num_blocks=num_blocks, inference=inference)
...
def EfficientNetB0(include_top=True, input_shape=None, input_tensor=None, weights=None, classes=1000, pooling=None, initial_inputs=None, num_blocks=None, inference=False):
    return _get_model_by_name('efficientnet-b0', include_top=include_top, input_shape=input_shape,
                              input_tensor=input_tensor, weights=weights, classes=classes, pooling=pooling, initial_inputs=initial_inputs, num_blocks=num_blocks, inference=inference)


def EfficientNetB1(include_top=True, input_shape=None, input_tensor=None, weights=None, classes=1000, pooling=None, initial_inputs=None, num_blocks=None, inference=False):
    return _get_model_by_name('efficientnet-b1', include_top=include_top, input_shape=input_shape,
                              input_tensor=input_tensor, weights=weights, classes=classes, pooling=pooling, initial_inputs=initial_inputs, num_blocks=num_blocks, inference=inference)


def EfficientNetB2(include_top=True, input_shape=None, input_tensor=None, weights=None, classes=1000, pooling=None, initial_inputs=None, num_blocks=None, inference=False):
    return _get_model_by_name('efficientnet-b2', include_top=include_top, input_shape=input_shape,
                              input_tensor=input_tensor, weights=weights, classes=classes, pooling=pooling, initial_inputs=initial_inputs, num_blocks=num_blocks, inference=inference)


def EfficientNetB3(include_top=True, input_shape=None, input_tensor=None, weights=None, classes=1000, pooling=None, initial_inputs=None, num_blocks=None, inference=False):
    return _get_model_by_name('efficientnet-b3', include_top=include_top, input_shape=input_shape,
                              input_tensor=input_tensor, weights=weights, classes=classes, pooling=pooling, initial_inputs=initial_inputs, num_blocks=num_blocks, inference=inference)


def EfficientNetB4(include_top=True, input_shape=None, input_tensor=None, weights=None, classes=1000, pooling=None, initial_inputs=None, num_blocks=None, inference=False):
    return _get_model_by_name('efficientnet-b4', include_top=include_top, input_shape=input_shape,
                              input_tensor=input_tensor, weights=weights, classes=classes, pooling=pooling, initial_inputs=initial_inputs, num_blocks=num_blocks, inference=inference)


def EfficientNetB5(include_top=True, input_shape=None, input_tensor=None, weights=None, classes=1000, pooling=None, initial_inputs=None, num_blocks=None, inference=False):
    return _get_model_by_name('efficientnet-b5', include_top=include_top, input_shape=input_shape,
                              input_tensor=input_tensor, weights=weights, classes=classes, pooling=pooling, initial_inputs=initial_inputs, num_blocks=num_blocks, inference=inference)


def EfficientNetB6(include_top=True, input_shape=None, input_tensor=None, weights=None, classes=1000, pooling=None, initial_inputs=None, num_blocks=None, inference=False):
    return _get_model_by_name('efficientnet-b6', include_top=include_top, input_shape=input_shape,
                              input_tensor=input_tensor, weights=weights, classes=classes, pooling=pooling, initial_inputs=initial_inputs, num_blocks=num_blocks, inference=inference)


def EfficientNetB7(include_top=True, input_shape=None, input_tensor=None, weights=None, classes=1000, pooling=None, initial_inputs=None, num_blocks=None, inference=False):
